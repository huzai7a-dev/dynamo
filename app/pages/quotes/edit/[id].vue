<template>
    <div class="min-h-screen bg-light-gray">
      <div class="container py-8">
        ...
        <div v-else>
          <OrderForm
            v-if="quote && !pending && dataSourceType === DataSource.ORDER" 
            :orderData="quote"
            :is-edit-mode="true"
            @success="()=> handleSuccess(DataSource.ORDER)"
            @error="handleError"
          />

          <VectorForm
            v-if="quote && !pending && dataSourceType === DataSource.VECTOR"
            :vectorData="quote"
            :isEditMode="true"
            @success="()=> handleSuccess(DataSource.VECTOR)"
            @error="handleError"
          />
        </div>
      </div>

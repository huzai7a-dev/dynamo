<template>
  <div>
    <QuotesTable
      ...
      @row-click="
        $router.push(
          `/quotes/${$event.row.id}?type=${($event.row as any).original_q_type}`,
        )
      "
    />
  </div>
...
const params = computed(() => ({
  ...
  ...(dateRange.value.from && {
    date_from: dateRange.value.from.toISOString().split("T")[0],
  }),
  ...(dateRange.value.to && {
    date_to: dateRange.value.to.toISOString().split("T")[0],
  }),
}));

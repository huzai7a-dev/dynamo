<template>
  <div class="flex flex-col gap-6">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        <h3 class="text-2xl font-black text-secondary tracking-tight">Paid Invoices</h3>
        <p class="text-sm text-charcoal/50 font-medium">View and manage your paid invoices.</p>
      </div>
    </div>
    ...
    <div v-else-if="error" class="flex items-center justify-center p-4">
      <div class="bg-rose-50 text-rose-600 p-8 rounded-3xl border border-rose-100 text-center max-w-md shadow-sm">
        ...
        <p class="text-sm opacity-80 font-medium">We couldn't load your paid invoices. Please try refreshing.</p>
      </div>
    </div>
    ...
    <div v-else>
      ...
      <template v-else>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            <div class="flex items-center gap-3 mb-3">
              ...
              <p class="text-xs font-medium text-charcoal/50 uppercase tracking-wide">Total Paid Invoices</p>
            </div>
            <p class="text-3xl font-black text-secondary">{{ stats.totalPaidInvoices }}</p>
          ...
          <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            <div class="flex items-center gap-3 mb-3">
              ...
              <p class="text-xs font-medium text-charcoal/50 uppercase tracking-wide">Total Amount Paid</p>
            </div>
            <p class="text-3xl font-black text-primary">${{ stats.totalAmountPaid }}</p>
            <p class="text-xs text-charcoal/40 mt-1">All time</p>
          </div>
          ...
          <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            <div class="flex items-center gap-3 mb-3">
              ...
              <p class="text-xs font-medium text-charcoal/50 uppercase tracking-wide">Latest Paid Invoice</p>
            </div>
            <p class="text-2xl font-black text-secondary">{{ formatDatePrimary(stats.latestPaidAt) }}</p>
            <p class="text-xs text-charcoal/40 mt-1">{{ formatDateSecondary(stats.latestPaidAt) }}</p>
          </div>
          ...
          <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            <div class="flex items-center gap-3 mb-3">
              ...
              <p class="text-xs font-medium text-charcoal/50 uppercase tracking-wide">Payment Success Rate</p>
            </div>
            <p class="text-3xl font-black text-primary">{{ stats.successRate }}%</p>
          ...
          <table class="min-w-full table-auto border-collapse text-sm font-sans">
            <thead class="bg-white border-b border-slate-200">
              <tr>
                ...
                <th class="px-5 py-4 text-center text-xs font-bold text-charcoal/70 uppercase tracking-widest">
                  <div class="flex items-center justify-center gap-1.5">
                    <Icon name="CheckCircle" class="w-3.5 h-3.5 text-primary/50" />
                    <span>Paid On</span>
                  </div>
                </th>
                <th class="px-5 py-4 text-center text-xs font-bold text-charcoal/70 uppercase tracking-widest">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-5 flex items-center gap-4 mt-6 shadow-sm">
          <div class="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center flex-shrink-0">
            <Icon name="CheckCircle" class="w-5 h-5 text-emerald-600" />
          </div>
          <div class="flex-1">
            <p class="text-sm font-bold text-secondary">All invoices are paid!</p>
            <p class="text-xs text-charcoal/50 font-medium">Thank you for your business. Keep up the great work!</p>
          </div>
          <!-- Decorative -->
          <div class="hidden sm:block">
            <svg width="80" height="50" viewBox="0 0 80 50" fill="none" xmlns="http://www.w3.org/2000/svg">
              <rect x="10" y="5" width="40" height="30" rx="3" fill="#f0fdf4" stroke="#bbf7d0" stroke-width="1"/>
              <line x1="18" y1="15" x2="42" y2="15" stroke="#d1fae5" stroke-width="2" stroke-linecap="round"/>
              <line x1="18" y1="22" x2="36" y2="22" stroke="#d1fae5" stroke-width="2" stroke-linecap="round"/>
              <rect x="35" y="18" width="35" height="24" rx="3" fill="#dcfce7" stroke="#86efac" stroke-width="1"/>
              <line x1="42" y1="26" x2="62" y2="26" stroke="#bbf7d0" stroke-width="2" stroke-linecap="round"/>
              <line x1="42" y1="33" x2="56" y2="33" stroke="#bbf7d0" stroke-width="2" stroke-linecap="round"/>
            </svg>
          </div>
        </div>
      </template>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
          ...
          <tbody>
            <tr v-for="(item, idx) in pdfItems" :key="idx" :style="{ background: idx % 2 === 0 ? '#f8fafc' : '#ffffff', borderBottom: '1px solid #e2e8f0' }">
              ...
              <td style="padding: 12px; text-align: center; font-size: 13px; font-weight: 600; color: #0f172a;">

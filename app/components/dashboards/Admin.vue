<template>
    <div class="bg-gray-50/50 min-h-screen">
        <!-- Search Section -->
        <div class="grid grid-cols-1 md:grid-cols-3 mb-8 gap-8 bg-white rounded-2xl shadow-sm border border-gray-100">
            <div class="space-y-3">
                <label class="text-sm font-bold text-gray-700 uppercase tracking-tight">Quote Number</label>
                <div class="flex gap-2">
                    <Input v-model="searchQuotes" placeholder="Search..." class="flex-1" size="sm" />
                    <Button rounded variant="primary" size="sm" class="px-6"
                        @click="handleSearch(DataSource.QUOTE, searchQuotes)">Search</Button>
                </div>
            </div>
            <div class="space-y-3">
                <label class="text-sm font-bold text-gray-700 uppercase tracking-tight">Order Number</label>
                <div class="flex gap-2">
                    <Input v-model="searchOrders" placeholder="Search..." class="flex-1" size="sm" />
                    <Button rounded variant="primary" size="sm" class="px-6"
                        @click="handleSearch(DataSource.ORDER, searchOrders)">Search</Button>
                </div>
            </div>
            <div class="space-y-3">
                <label class="text-sm font-bold text-gray-700 uppercase tracking-tight">Vector Number</label>
                <div class="flex gap-2">
                    <Input v-model="searchVectors" placeholder="Search..." class="flex-1" size="sm" />
                    <Button rounded variant="primary" size="sm" class="px-6"
                        @click="handleSearch(DataSource.VECTOR, searchVectors)">Search</Button>
                </div>
            </div>
        </div>
        ...
        <div
            ...
            <!-- Quotes Column -->
            <div class="flex flex-col border-r border-gray-100 lg:border-gray-200 last:border-r-0">
                <div class="bg-[#f8f9fa] px-6 py-4 border-b border-gray-200">
                    <h2 class="text-xl font-bold text-gray-800 tracking-tight">Quotes</h2>
                </div>
                <div class="p-4 space-y-1">
                    <button v-for="option in quoteOptions" :key="option.id" @click="selectOption('quote', option)"
                        :class="[
            ...
            <div class="flex flex-col border-r border-gray-100 lg:border-gray-200 last:border-r-0">
                <div class="bg-[#f8f9fa] px-6 py-4 border-b border-gray-200">
                    <h2 class="text-xl font-bold text-gray-800 tracking-tight">Orders</h2>
                </div>
                <div class="p-4 space-y-1">
                    <button v-for="option in orderOptions" :key="option.id" @click="selectOption('order', option)"
                        :class="[
                            'w-full text-left px-5 py-3.5 rounded-xl transition-all duration-300 flex justify-between items-center group',
                            activeOption?.id === option.id
                                ? 'bg-primary/10 text-primary font-bold shadow-sm'
            ...
            <div class="flex flex-col">
                <div class="bg-[#f8f9fa] px-6 py-4 border-b border-gray-200">
                    <h2 class="text-xl font-bold text-gray-800 tracking-tight">Vectors</h2>
                </div>
                <div class="p-4 space-y-1">
                    <button v-for="option in vectorOptions" :key="option.id" @click="selectOption('vector', option)"
                        :class="[
                            'w-full text-left px-5 py-3.5 rounded-xl transition-all duration-300 flex justify-between items-center group',
                            activeOption?.id === option.id
                                ? 'bg-primary/10 text-primary font-bold shadow-sm'
        ...
        <div v-if="activeOption" class="mt-8 transition-all duration-500 animate-in fade-in slide-in-from-bottom-4">
            <UiTable class="h-[calc(100vh-500px)]" :loading="isLoading" :data="data" :columns="columns" infinite-scroll
                :pagination="pagination" @load-more="loadMore" @row-click="$router.push(`/orders/${$event.row.id}`)">
...
import { DataSource, OrderStatus } from '../../../shared/types/enums'
...
const activeOption = ref<any>(null)
const isLoading = ref(false)
const data = ref<TableOrders[]>([])
...
const columns = computed(() => {
    return [
        ...
        { key: activeOption.value.type === DataSource.ORDER ? 'order_name' : 'vector_name', label: 'Order Name' },
...
const handleSearch = async (type: DataSource, search: string) => {
    if (type === DataSource.ORDER || type === DataSource.VECTOR) {
...
const selectOption = async (type: string, option: any) => {
...
const loadMore = async () => {
    if (isLoading.value || pagination.value.currentPage >= pagination.value.totalPage) return
...
const fetchData = async (append = false, type: DataSource) => {
    ...
    if (activeOption.value.action === 'search') {
        params.order_number = searchOrders.value
    } else {
    ...
    const url = type === DataSource.ORDER ? '/api/orders' : type === DataSource.VECTOR ? '/api/vectors' : '/api/quotes'
    try {
        ...
        const tableData = type === DataSource.ORDER ? response.data.orders : type === DataSource.VECTOR ? response.data.vectors : response.data.quotes

<template>
  <div class="min-h-screen">
      ...
      <div v-for="config in searchConfigs" :key="config.type" class="space-y-3">
        ...
        <div class="flex gap-2">
          ...
          <Button
            ...
            size="sm"
            class="px-6"
            @click="handleSearch(config.type, config.model.value)"
            >Search</Button
          >
        </div>
      </div>
    ...
    >
      <div
        v-for="(group, index) in dashboardConfig"
        :key="group.title"
      ...
      >
        ...
        <div class="p-4 space-y-1">
          <button
            ...
            :key="option.id"
            @click="selectOption(group.type, option)"
            :class="[
    ...
    <div
      v-if="activeOption"
      class="mt-8 transition-all duration-500 animate-in fade-in slide-in-from-bottom-4"
    >
      <UiTable
        class="h-[calc(100vh-500px)]"
        :loading="isLoading"
        :data="data"
        :columns="columns"
        infinite-scroll
      ...
      >
        ...
        <template #column-view="{ row }">
          <button
            ...
            title="View Details"
            @click="
              $router.push(`/${activeOption?.type}s/${(row as TableOrders).id}`)
            "
          >
        ...
        <template #column-actions="{ row }">
          <OrderActions
            v-if="activeOption?.type !== DataSource.QUOTE"
            size="compact"
            :isAdmin="true"
            :status="(row as TableOrders).status"
            @approve="handleApprove(row as TableOrders)"
            @reject="handleReject(row as TableOrders)"
            @deliver="openDeliver(row as TableOrders)"
          />
        </template>
          ...
          <span
            ...
            :class="
              getPaymentStatusBadgeClass(
                ...
                String((row as TableOrders)?.price || 0),
              )
            "
          >
            {{
        ...
        <template #column-created_at="{ row }">
          <span class="text-primary font-semibold">{{
            formateDate((row as TableOrders).created_at)
          }}</span>
        </template>

        <template #column-rush="{ row }">
          <span v-if="row.rush === 'Yes'" class="text-primary font-semibold">
            New/<span class="text-red-600">Rush</span>
          </span>
          <span v-else class="text-primary font-semibold">New</span>
        </template>

        <template #column-q_type="{ row }">
          <span class="text-primary font-semibold">{{
            row?.q_type?.toUpperCase()
          }}</span>
        </template>
      </UiTable>
      ...
      <DeliveryModal
        ...
        :orderId="selectedRow ? String(selectedRow.id) : ''"
        :orderDate="selectedRow?.created_at || ''"
        :userId="selectedRow?.user_id ?? ''"
        :loading="delivering"
        @on:deliver="handleDeliverySubmit"
      />
    </div>
  </div>
</template>
...
const dashboardConfig: DataGroup[] = [
  ...
  },
  {
    ...
    options: [
      ...
      {
        id: "o4",
        label: "Orders with price 0",
        action: "fetchPrice0",
        type: DataSource.ORDER,
      },
      {
        id: "o5",
        label: "Orders with Price",
        action: "fetchWithPrice",
        type: DataSource.ORDER,
      },
    ],
  },
  {
    ...
    options: [
      ...
      {
        id: "v4",
        label: "Vectors with price 0$",
        action: "fetchPrice0",
        type: DataSource.VECTOR,
      },
      {
        id: "v5",
        label: "vectors with Price",
        action: "fetchWithPrice",
        type: DataSource.VECTOR,
      },
    ],
...
const selectOption = async (type: DataSource, option: Option) => {
  ...
  if ([DataSource.ORDER, DataSource.VECTOR, DataSource.QUOTE].includes(type)) {
    ...
    console.log(
      `Fetching data for ${type}: ${option.action} (Not implemented yet)`,
    );
  }
};
...
// ─── Row actions (approve/reject/deliver) ──────────────────────────────────
const showDeliveryModal = ref(false);
const delivering = ref(false);
const selectedRow = ref<TableOrders | null>(null);

const handleApprove = async (row: TableOrders) => {
  if (!activeOption.value) return;
  const { updateStatus } = useOrderVectorActions(
    activeOption.value.type === DataSource.VECTOR ? "vector" : "order",
  );
  const success = await updateStatus(row.id, "approve");
  if (success) await fetchData(false, activeOption.value.type);
};

const handleReject = async (row: TableOrders) => {
  if (!activeOption.value) return;
  const { updateStatus } = useOrderVectorActions(
    activeOption.value.type === DataSource.VECTOR ? "vector" : "order",
  );
  const success = await updateStatus(row.id, "reject");
  if (success) await fetchData(false, activeOption.value.type);
};

const openDeliver = (row: TableOrders) => {
  selectedRow.value = row;
  showDeliveryModal.value = true;
};

const handleDeliverySubmit = async (formData: any) => {
  if (!activeOption.value || !selectedRow.value) return;
  delivering.value = true;
  try {
    const { deliver } = useOrderVectorActions(
      activeOption.value.type === DataSource.VECTOR ? "vector" : "order",
    );
    const success = await deliver(selectedRow.value.id, formData);
    if (success) {
      showDeliveryModal.value = false;
      await fetchData(false, activeOption.value.type);
    }
  } finally {

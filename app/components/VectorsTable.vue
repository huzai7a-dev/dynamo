<template>
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
      <TableHeader
        description="Filter by vector number, vector name, and date range"
        firstPlaceholder="Search by vector number"
        secondPlaceholder="Search by vector name"
      ...
      <div class="p-6">
        ...
        >
          ...
          <template #column-vector_name="{ row }">
            {{
              row?.is_from_quote
                ? `${row.vector_name} (QR-${row?.from_quote_id})`
                : row.vector_name
            }}
          </template>
            ...
            <button
              ...
              :title="
                (row as TableOrders).status === OrderStatus.DELIVERED
                  ? 'Delivered vectors cannot be edited'
                  : 'Edit this vector'
              "
        ...
        <EntityDetailModal
          ...
          type="vector"
          :entityId="String(selectedVectorId)"
...
const handleDownloadFiles = async (id: string | number) => {
  try {
    const blob = await $fetch<Blob>(`/api/attachments/vector/${id}/download`, {
      responseType: "blob",
    });
    downloadBlob(blob, `vector-${id}-files.zip`);
  } catch (err: any) {

...
const isImage = (a: any) =>
  (a.resource_type || "").toLowerCase() === "image" ||
  IMAGE_FORMATS.has((a.format || "").toLowerCase());
...
const handleDownload = async (a: Attachment) => {
  downloadingUrl.value = a.url;
  try {
    const res = await fetch(a.url);
    if (!res.ok) throw new Error("Download failed");
    const blob = await res.blob();
    downloadBlob(blob, displayName(a));
  } catch {
...
<template>
  <div>
    <h2 v-if="title" class="mb-4 text-lg font-semibold text-secondary">
      {{ title || "Attachments" }}
    </h2>
    ...
    <div
      v-else
      class="grid auto-rows-fr grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6"
    >
      <article
      ...
      >
        ...
        <div class="relative w-full bg-slate-50 aspect-video">
          ...
          <div
            v-else
            class="absolute inset-0 flex flex-col items-center justify-center gap-1 p-1"
          >
            <div
              class="grid h-8 w-8 place-items-center rounded-lg bg-primary-light text-primary-dark"
            >
              <svg
        ...
        <div class="p-2">
          <div
            class="truncate text-[10px] font-bold text-secondary"
            :title="displayName(a)"
          >
            {{ displayName(a) }}
          </div>
        ...
        <div
          class="mt-auto flex items-center justify-between border-t border-slate-200 bg-slate-50/40 p-2"
        >
          <button
            type="button"
            :disabled="downloadingUrl === a.url"
            @click="handleDownload(a)"
            class="w-full inline-flex items-center justify-center gap-1 rounded-lg bg-primary px-2 py-1 text-[9px] font-bold text-white transition hover:bg-primary-dark disabled:opacity-60"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-2.5 w-2.5"

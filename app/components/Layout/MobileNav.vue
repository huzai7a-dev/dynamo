<template>
  <div 
    :class="[
      'fixed inset-0 bg-secondary z-40 flex flex-col pt-12 px-6 md:hidden transition-all duration-300 ease-in-out',
  ...
  >
    ...
    <template v-for="item in navItems" :key="item.label">
      ...
      <NuxtLink v-else :key="item.url" :to="item.url" class="py-4 text-lg font-medium text-light-gray hover:text-white border-b border-gray-700" @click="$emit('close')">{{ item.label }}</NuxtLink>
    </template>
  </div>
</template>

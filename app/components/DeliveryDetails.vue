  ...
  <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    ...
    <div class="mb-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <OrderKeyValue v-if="deliveryData.discount" label="Discount"
          :value="deliveryData.discount ? `$${deliveryData.discount}` : undefined" />
        <OrderKeyValue v-if="deliveryData.total_price" label="Total Price"
          :value="deliveryData.total_price ? `$${deliveryData.total_price}` : undefined" />
        <OrderKeyValue v-if="deliveryData.is_free !== undefined" label="Free Order"
          :value="deliveryData.is_free ? 'Yes' : 'No'" />
        <OrderKeyValue v-if="deliveryData.designer_level" label="Designer Level" :value="deliveryData.designer_level" />
        <OrderKeyValue v-if="deliveryData.assign_percentage" label="Assign Percentage"
    ...
    <div v-if="deliveryData.height || deliveryData.width" class="mb-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <OrderKeyValue v-if="deliveryData.height" label="Height" :value="deliveryData.height" />
        <OrderKeyValue v-if="deliveryData.width" label="Width" :value="deliveryData.width" />
        <OrderKeyValue v-if="deliveryData.height && deliveryData.width" label="Dimensions"
    ...
    <div v-if="deliveryData.customer_requirement" class="mb-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <OrderKeyValue v-if="deliveryData.customer_requirement.normal_delivery" label="Normal Delivery"
          :value="deliveryData.customer_requirement.normal_delivery" />
        <OrderKeyValue v-if="deliveryData.customer_requirement.edit_or_change" label="Edit or Change"
          :value="deliveryData.customer_requirement.edit_or_change" />
        <OrderKeyValue v-if="deliveryData.customer_requirement.edit_in_stitch_file" label="Edit in Stitch File"
    ...
    <div v-if="hasAdditionalComments" class="mb-6">
      ...
      <div class="space-y-4">
        <div v-if="deliveryData.customer_requirement?.comment_box_1" class="bg-gray-50 rounded-lg p-4">
          ...
          <p class="text-sm text-secondary whitespace-pre-wrap">{{ deliveryData.customer_requirement.comment_box_1 }}
          </p>
        </div>
        <div v-if="deliveryData.customer_requirement?.comment_box_2" class="bg-gray-50 rounded-lg p-4">
          ...
          <p class="text-sm text-secondary whitespace-pre-wrap">{{ deliveryData.customer_requirement.comment_box_2 }}
          </p>
        </div>
        <div v-if="deliveryData.customer_requirement?.comment_box_3" class="bg-gray-50 rounded-lg p-4">
          ...
          <p class="text-sm text-secondary whitespace-pre-wrap">{{ deliveryData.customer_requirement.comment_box_3 }}
          </p>
        </div>
        <div v-if="deliveryData.customer_requirement?.comment_box_4" class="bg-gray-50 rounded-lg p-4">
          ...
          <p class="text-sm text-secondary whitespace-pre-wrap">{{ deliveryData.customer_requirement.comment_box_4 }}
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-if="deliveryData.created_at" class="mb-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <OrderKeyValue label="Delivered At" :value="new Date(deliveryData.created_at).toLocaleString()" />
      </div>
    </div>
  </div>
</template>

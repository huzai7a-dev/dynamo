import vectorsService from "~~/server/services/vector.service";
import { parseMultipart } from "~~/server/utils/multiplart";

export default defineEventHandler(async (event) => {
  try {
    const vectorId = getRouterParam(event, "id") as string;
    ...
    // Update the order
    const result = await vectorsService.updateVector(
      userId,
      parseInt(vectorId),
      fields,
      files,
